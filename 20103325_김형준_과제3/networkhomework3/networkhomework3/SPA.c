...
void send_path(SOCKET tests, graph_t *g) {
	int n, j;
	vertex_t *v, *u;

	for (int k = 0; k < g->vertices_len; k++)
	{
		char sm1[4] = { 0 }, sm2[4] = { 0 }, sm3[4] = { 0 };
		char msg[100];
		int tmp = 0;
		v = g->vertices[k];
		...
		sprintf(msg, "%s %s %s\0", &sm1, &sm2, &sm3);
		printf("%s", msg);
//		send(tests, msg, BUFSIZE12, 0);
...
int main() {
	...
	char testAdd[100], studentAdd[100], name[20], ID[20], msg[100];
	char testPort[10], studentPort[10];
	struct timeval start, end;


	while (1)
	{
		printf("mode(1=graph generation, 5=client, 6=online graph server,7=test for server, 8=student-version server, 9=quit): ");
		scanf("%d", &mode);
		if (mode == 9) break;
		switch (mode)
		{
		...
		case 8:   //구현해야 할 것.
			...
			scanf("%s %s", &testAdd, &testPort);

			printf("student-server-IP-address, student-server-port#:, 이름, 학번: ");
			scanf("%s %s %s %s", &studentAdd, &studentPort, &name, &ID);
			...
			studentAddr.sin_addr.s_addr = inet_addr(studentAdd);
			studentAddr.sin_port = htons(atoi(studentPort));
			studentSock = socket(PF_INET, SOCK_STREAM, 0);
			if (studentSock == INVALID_SOCKET)
				ErrorHandling("student socket() error");

			hMutex = CreateMutex(NULL, FALSE, NULL);
			if (hMutex == NULL) {
				ErrorHandling("CreateMutex() error");
			}
			if (bind(studentSock, (SOCKADDR*)&studentAddr, sizeof(studentAddr)) == SOCKET_ERROR)
				ErrorHandling("bind() error");
			if (listen(studentSock, 5) == SOCKET_ERROR)
				ErrorHandling("listen() error");

			// server to server connection test서버로 연결됨.
			memset(&testAddr, 0, sizeof(testAddr));
			testAddr.sin_family = AF_INET;
			testAddr.sin_addr.s_addr = inet_addr(testAdd);
			testAddr.sin_port = htons(atoi(testPort));
			testSock = socket(PF_INET, SOCK_STREAM, 0); //IP4V, TCP전송 
			if (testSock == INVALID_SOCKET)
				ErrorHandling("test socket() error");
			//test서버 연결.
			if ((connect(testSock, (SOCKADDR *)&testAddr, sizeof(testAddr))) == SOCKET_ERROR)
				ErrorHandling("test server connect() error!!");
			//test서버에 studentAdd, studentPort, name, ID를 보냄
			sprintf(msg, "%s %s %s %s\0", &studentAdd, studentPort, name, ID);
			send(testSock, msg, strlen(msg), 0);
			//test서버로부터 받은 값. studentAdd, name, ID를 받음
			//         printf("서버로부터의 응답: ");
			//         recv(testSock, msg, BUFSIZE, 0);
			//         printf(msg);

			while (1) {
				//test서버로부터의 답변을 기다림.
				clntAddrSize = sizeof(clntAddr);
				clntSock = accept(studentSock, (SOCKADDR*)&clntAddr, &clntAddrSize);
				...
				//test서버에 노드/시작노드/링크수를 보냄.
				//노드 갯수
				memset(sss, 0, sizeof(char)*BUFSIZE4);
				for (i = 3; i > 0; i--) {
					//노드수 numvert, 시작노드 startID, 링크수 numedge
					sss[i] = numvert&FF;
					numvert >>= 8;
				}
				send(testSock, sss, BUFSIZE4, 0);
				//시작노드
				for (i = 3; i > 0; i--) {
					sss[i] = startID&FF;
					startID >>= 8;
